...
class Userctrl {
        ...
        try {
            ...
            if (res.length == 0) {
                ...
                if (res[0].password !== oldPassword) {
                    ...
                    try {
                        const res = await query(`UPDATE user_table a SET PASSWORD= ? WHERE a.student_num = ? `, [
                            newPassword,
                            studentNum,
                        ]);
                        ctx.status = 200;
    ...
    static async completeusr(ctx) {
        ...
        if (token && token !== null) {
            ...
            const existThis = await query(
                `SELECT student_num FROM user_info_table 
                WHERE student_num = ?`,
                [userNum],
            );
                ...
                try {
                    ...
                    try {
                        const res = await query(
                            `UPDATE user_info_table set description=?,
                        depart_num=?,major_num=?,image=?,phone_num=?,email=? where student_num=?`,
                            values,
                        );
                        try {
    ...
    static async userInfo(ctx) {
        ...
        if (token && token !== null) {
            ...
            try {
                ...
                try {
                    const reserchRes = await query(
                        `SELECT a.type_num FROM reserch_table a WHERE a.student_num = ? GROUP BY a.type_num`,
                        values,
                    );
                    res.status = 200;
                    ctx.body = {
